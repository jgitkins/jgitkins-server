...
import io.jgitkins.server.application.port.in.CreateJobUseCase;
...
import io.jgitkins.server.domain.repository.JobRepository;
...
public class CreateJobService implements CreateJobUseCase {
    ...
    public void createJob(JobCreateCommand command) {
        ...
        boolean hasJenkinsFile = checkFileExistencePort.exists(
                command.getTaskCd(),
                command.getRepoName(),
        ...
        if (!hasJenkinsFile) {
            log.info("Jenkinsfile not found in repo: {}, commit: {}. Skipping job creation.",
                    command.getRepoName(), command.getCommitHash());
            return;
        }

        log.info("Jenkinsfile found. Creating job for repo: {}, commit: {}",
                command.getRepoName(), command.getCommitHash());

        // 2. Job 도메인 객체 생성
        Job job = Job.create(
                RepositoryId.of(command.getRepoName()), // TODO: Repo ID 매핑 필요시 수정

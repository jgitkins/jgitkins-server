...
class RepositoryProvisionedEventListenerTest {
    ...
    @Mock
    private OrganizePersistencePort organizePersistencePort;
    @Mock
    private RepositoryContentPort repositoryContentPort;
    @Mock
    private RepositoryCommitPort repositoryCommitPort;
    @Mock
    private UpdateHeadReferencePort updateHeadReferencePort;
    @Mock
    ...
    void shouldHandleInitialProvisioningWhenReadmeRequested() {
        ...
        when(organizePersistencePort.findById(OrganizeId.of(1L))).thenReturn(Optional.of(organize));
        List<CommitFile> files = List.of(new CommitFile("README.md", "hello".getBytes()));
        when(repositoryContentPort.prepareInitialFiles("demo")).thenReturn(files);
        when(repositoryPersistencePort.findByOrganizeAndName(OrganizeId.of(1L), RepositoryName.from("demo")))
                .thenReturn(Optional.of(repository));
        Repository synced = repository.markInit(LocalDateTime.now());
        when(repositoryPersistencePort.update(any(Repository.class))).thenReturn(synced);
        ...
        verify(repositoryPersistencePort).update(any(Repository.class));
    ...
    void shouldSkipWhenInitialContentNotRequired() {
        ...
        when(organizePersistencePort.findById(OrganizeId.of(2L))).thenReturn(Optional.of(organize));
        when(repositoryPersistencePort.findByOrganizeAndName(OrganizeId.of(2L), RepositoryName.from("demo")))
                .thenReturn(Optional.of(repository));
        Repository synced = repository.markInit(LocalDateTime.now());
        when(repositoryPersistencePort.update(any(Repository.class))).thenReturn(synced);
        ...
        verify(repositoryContentPort, never()).prepareInitialFiles(any());

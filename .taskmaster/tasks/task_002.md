...
<info added on 2026-03-12T05:05:57.786Z>
PushEventCommand에서 org.eclipse.jgit.receive.ReceiveCommand 및 org.eclipse.jgit.lib.Constants 의존과 gitDirPath 기반 식별을 제거하고, `PushHookController`가 `PushHookRequest`를 처리하는 inbound adapter(`adapter.in.web.webhook`) 측에 `PushEventCommandMapper`를 도입하여 JGit 명령 해석과 저장소 식별 책임을 이동함. `PushEventHandleService`는 순수 command(repositoryId, taskCd, repoName) 기반 오케스트레이션만 담당하도록 정리함. `PushEventHandleServiceTest`와 `PushEventCommandMapperTest`로 회귀 검증을 추가하고 ArchitecturePackageConventionTest를 포함한 최소 회귀 세트를 통과함.
</info added on 2026-03-12T05:05:57.786Z>
